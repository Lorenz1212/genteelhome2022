...
class Datatable_model extends CI_Model{  
    ...
    function Design_Project_Rejected_DataTable($user_id){
     ...
     function Customization_DataTable($user_id,$status){
        if($status == 'REQUEST'){
            $array = array('s.status'=>$status, 's.type'=>'customized');
        }else{
            $array = array('s.status'=>$status, 's.type'=>'customized','s.designer'=> $user_id);
        }

        $query =  $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y %r") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where($array)->order_by('date_created','DESC')->get();
     ...
     function Customization_Request_DataTable($user_id,$status){
         if($status == 'REQUEST'){
            $array = array('s.status'=>$status, 's.type'=>'request');
        }else{
            $array = array('s.status'=>$status, 's.type'=>'request','s.designer'=> $user_id);
        }
         $query =  $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y %r") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where($array)->order_by('date_created','DESC')->get();
        if($query !== FALSE && $query->num_rows() > 0){
           foreach($query->result() as $row){
        ...
        function OnlineRequest_DataTable($user_id,$status){
        if($status == 'REQUEST'){
            $array = array('i.type'=>'REQUEST', 'c.designer'=> $user_id);
        }else{
            $array = array('i.designer_status'=>'APPROVED','c.designer'=> $user_id);
        }

         $query =  $this->db->select('s.*,i.*,c.*,d.*,u.*,d.title as title,c.c_name as color,s.status as status,
            i.price as price,i.type as type,i.id as item_id,s.type as s_type,c.image as image,
          ...
          ->join('tbl_project_design as d','d.project_no=i.project_no','LEFT')
          ->join('tbl_project_color as c','c.project_no=d.id','LEFT')
          ->join('tbl_users as u','u.id=s.sales','LEFT')
          ->where($array)->order_by('s.date_order','DESC')
          ->get();
      
        if($query !== FALSE && $query->num_rows() > 0){
           foreach($query->result() as $row){
            if($status == 'REQUEST'){
                $action = '<button type="button" class="btn btn-sm btn-circle btn-success btn-icon" id="save_approved" data-id="'.$row->item_id.'" data-action="DESIGNER_APPROVED"><i class="icon-xs  ki ki-bold-check"></i></button>'; 
            }else{
                $action = '<button type="button" class="btn btn-sm btn-circle btn-success btn-icon" disabled><i class="icon-xs  ki ki-bold-check"></i></button>';    
            }
        ...
        function RawMaterial_DataTable(){
           $query = $this->db->select('*,DATE_FORMAT(date_created, "%M %d %Y %r") as date_created')->from('tbl_materials')->order_by('id','DESC')->get();  
            if($query !== FALSE && $query->num_rows() > 0){
               foreach($query->result() as $row){
                $action = '<button type="button" class="btn btn-sm btn-dark btn-shadow btn-icon" data-id="'.$row->id.'" id="form-request" data-toggle="modal" data-target="#exampleModal"><i class="la la-eye"></i></button>'; 
                    $data[] = array(
                             'no'           => $row->item_no,
    ...
    function RawMaterial_Stocks_DataTable(){
        ...
        if($query !== FALSE && $query->num_rows() > 0){
           foreach($query->result() as $row){
            $action = '<button type="button" class="btn btn-sm btn-dark btn-shadow btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->id.'" data-target="#requestModal"><i class="la la-eye"></i></button>';
               $data[] = array(
                      'no'           => $row->item_no,
                      'item'         => $row->item.' ('.$row->unit.')',
                      'stocks'       => $row->stocks,
     ...
     function RawMat_Production_Stocks_DataTable(){
        ...
        if($query !== FALSE && $query->num_rows() > 0){
           foreach($query->result() as $row){
             $action = '<button type="button" class="btn btn-sm btn-dark btn-shadow btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->id.'" data-target="#exampleModal"><i class="la la-eye"></i></button>';
               $data[] = array(
                      'no'           => $row->item_no,
                      'item'         => $row->item,
     ...
     function SpareParts_Stocks_DataTable(){
        ...
        if($query !== FALSE && $query->num_rows() > 0){
            $i=1;
           foreach($query->result() as $row){
               $action = '<button type="button" class="btn btn-sm btn-dark btn-shadow btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->id.'" data-target="#requestModal"><i class="flaticon2-pen"></i></button>';
               $data[] = array(
                      'no'           => $i,
                      'item'         => $row->item,
                      'stocks'       => $row->stocks,
    ...
    function OfficeSupplies_Received_DataTable($user_id,$status){
          ...
          if($query !== FALSE && $query->num_rows() > 0){
              foreach($query->result() as $row)  {
                if($row->status == 0){
                   ...
                   $action = '<button data-toggle="modal" data-target="#requestModal" id="form-request" data-id="'.$row->production_no.'" class="btn btn-sm btn-dark btn-shadow btn-icon" title="View Request"><i class="la la-eye"></i></button>';  
                   $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'"></div>';
     ...
     function Purchase_Material_Stocks_Inprogress_DataTable($user_id){
          ...
          if($query !== FALSE && $query->num_rows() > 0){
              foreach($query->result() as $row){
               $action = '<button data-toggle="modal" data-target="#processModal" id="form-request-inprogress" data-id="'.$row->fund_no.'" class="btn btn-sm btn-dark btn-shadow btn-icon" title="View Request"><i class="la la-eye"></i></button>';  
               $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'"></div>';
               $title = '<span style="width: 250px;"><div class="d-flex align-items-center"><div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/palettecolor/'.$row->c_image.'"></div><div class="ml-3"><div class="text-dark-75 font-weight-bolder font-size-lg mb-0">'.$row->title.'</div><a href="#" class="text-muted font-weight-bold text-hover-primary">'.$row->c_name.'</a></div></div></span>';
    ...
    function Purchase_Material_Project_Complete_DataTable($user_id){
      ...
      function Material_Request_Stocks_DataTable($user_id){       
         ...
         if($query !== FALSE && $query->num_rows() > 0){
            foreach($query->result() as $row)  
            {
               $action = ' <button data-toggle="modal" data-target="#modal-form" id="form-request-inprogress" data-id="'.$row->production_no.'" class="btn btn-icon btn-dark btn-hover-primark btn-sm mx-3" title="View Request"><i class="la la-eye"></i></button>';  
               $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'"></div>';
               $title = '<span style="width: 250px;"><div class="d-flex align-items-center"><div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/palettecolor/'.$row->c_image.'"></div><div class="ml-3"><div class="text-dark-75 font-weight-bolder font-size-lg mb-0">'.$row->title.'</div><a href="#" class="text-muted font-weight-bold text-hover-primary">'.$row->c_name.'</a></div></div></span>';
                     $data[] = array(
                          'production_no' => $row->production_no,
    ...
    function Joborder_Stocks_Request_DataTable($user_id){        
     ...
     function Joborder_Stocks_Pending_DataTable($user_id){        
        ...
        function Joborder_Project_Request_DataTable($user_id){        
         ...
         if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  {
                 $action = '<a  href="'.base_url().'gh/'.$this->session->userdata('page').'/joborder-update-project?URI='.base64_encode($row->production_no).'" class="btn btn-sm btn-dark btn-icon"><i class="flaticon2-pen"></i></a>'; 
     ...
     function Joborder_Stocks_Supervisor_DataTable(){
         ...
         if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row){
              $action = '<button type="button" class="btn btn-icon btn-dark btn-hover-success btn-sm mx-3" data-toggle="modal" id="form-request" data-id="'.$row->production_no.'" data-target="#requestModal"><i class="flaticon2-pen"></i></button>';
               $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'"></div>';
               $title = '<span style="width: 250px;"><div class="d-flex align-items-center"><div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/palettecolor/'.$row->c_image.'"></div><div class="ml-3"><div class="text-dark-75 font-weight-bolder font-size-lg mb-0">'.$row->title.'</div><a  class="text-muted font-weight-bold text-hover-primary">'.$row->c_name.'</a></div></div></span>';
                 $data[] = array(
                          'production_no' => $row->production_no,
    ...
    function Joborder_Admin_DataTable($user_id,$status)
       {        
          if($status == 'PENDING'){
             $array = array('p.status' => 'MATERIAL REQUEST','p.production' => $user_id);  
          }else{
             $array = array('p.status' => $status,'p.production' => $user_id);  
          }
         ...
         foreach($query->result() as $row)  
            {
               ...
               $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'" alt="'.$row->title.' ('.$row->c_name.')"></div>';
               $title = '<span style="width: 250px;"><div class="d-flex align-items-center"><div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/palettecolor/'.$row->c_image.'" alt="'.$row->c_name.'"></div><div class="ml-3"><div class="text-dark-75 font-weight-bolder font-size-lg mb-0">'.$row->title.'</div><a href="#" class="text-muted font-weight-bold text-hover-primary">'.$row->c_name.'</a></div></div></span>';
               if($row->status == 'COMPLETE'){$qty = $row->unit;}else{$qty = $row->balance;}  
                 $data[] = array(
                          'production_no' => $row->production_no,
                          'image'        => $image,
                          'title'        => $title,
       ...
       function Salesorder_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
             if($row->status == 'PENDING'){
                $action = '<a type="button" id="view_href" data-id="'.base64_encode($row->so_no).'" class="btn btn-sm btn-circle btn-primary btn-icon"><i class="la la-eye"></i></a>';   
            }else{
                $action = '<button type="button" class="btn btn-sm btn-circle btn-primary btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
            }
      ...
      function Salesorder_Request_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','REQUEST')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row){
            $action = '<button type="button" class="btn btn-sm btn-dark btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
            $data[] = array(
                      'so_no'        => $row->so_no,
                      'sales_person' => $row->sales_person,
                      'b_name'       => $row->c_name,
     ...
     function Salesorder_Pending_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','PENDING')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
             $action = '<a type="button" id="view_href" data-id="'.base64_encode($row->so_no).'" class="btn btn-sm btn-dark btn-icon"><i class="la la-eye"></i></a>';   
             $data[] = array(
                      'so_no'        => $row->so_no,
                      'sales_person' => $row->sales_person,
                      'b_name'       => $row->c_name,
     ...
     function Salesorder_Approved_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','REQUEST FOR APPROVAL')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
              $action = '<button type="button" class="btn btn-sm btn-dark btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
     ...
     function Salesorder_Shipping_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','SHIPPING')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
              $action = '<button type="button" class="btn btn-sm btn-dark btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
     ...
     function Salesorder_Delivered_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','DELIVERED')->where('s.sales_person', $user_id)->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
              $action = '<button type="button" class="btn btn-sm btn-dark btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
      ...
      function Salesorder_Return_DataTable($user_id,$status){
       $query = $this->db->select('s.*,d.*,c.*,o.*,s.qty as qty,s.status as status,DATE_FORMAT(o.date_delivered, "%M %d %Y") as date_created,
        CONCAT(u.firstname, " ",u.lastname) AS sales_person')
           ->from('tbl_salesorder_item_return as s')
           ->join('tbl_project_design as d','d.project_no=s.project_no','LEFT')
           ->join('tbl_project_color as c','c.project_no=d.id','LEFT')
           ->join('tbl_salesorder as o','o.so_no=s.so_no','LEFT')
           ->join('tbl_users as u','u.id=o.sales_person','LEFT')
           ->where('s.returner', $user_id)->group_by('s.so_no')->order_by('o.date_delivered','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
            $image = '<div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/design/project_request/images/'.$row->image.'" alt="'.$row->title.' ('.$row->c_name.')"></div>';
            $title = '<span style="width: 250px;"><div class="d-flex align-items-center"><div class="symbol symbol-40 symbol-circle symbol-sm"><img class="" id="myImg" src="'.base_url().'assets/images/palettecolor/'.$row->c_image.'" alt="'.$row->c_name.'"></div><div class="ml-3"><div class="text-dark-75 font-weight-bolder font-size-lg mb-0">'.$row->title.'</div><a href="#" class="text-muted font-weight-bold text-hover-primary">'.$row->c_name.'</a></div></div></span>'; 
            if($row->status == 'GOOD'){
                $status = '<span class="label label-lg label-light-success label-inline font-weight-bold py-4">GOOD</span>';
            }else{
                $status = '<span class="label label-lg label-light-danger label-inline font-weight-bold py-4">REJECTED</span>';
            }
             $data[] = array(
                      'so_no'        => $row->so_no,
                      'image'        => $image,
       ...
       function Salesorder_For_Shipping_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','SHIPPING')->order_by('date_created','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
            $action = '<button type="button" class="btn btn-sm btn-dark btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
     ...
     function Salesorder_For_Delivered_DataTable($user_id){
       $query = $this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','DELIVERED')->order_by('date_created','DESC')->get();
   ...
   function Approval_Inspection_Project_Request_DataTable(){
    ...
    ->order_by('i.date_created','DESC')->get();
      ...
      function Approval_Request_Salesorder_DataTable($user_id){
        $query=$this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y %r") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')
            ->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status', 'REQUEST FOR APPROVAL')->order_by('date_created','DESC')->get();
           if($query !== FALSE && $query->num_rows() > 0){
              foreach($query->result() as $row)  
            {
           $action = '<button type="button" class="btn btn-sm btn-circle btn-primary btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
     ...
     function Approval_Approved_Salesorder_DataTable($user_id){
        $query=$this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y %r") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')
            ->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.approval_status', 1)->order_by('date_created','DESC')->get();
           if($query !== FALSE && $query->num_rows() > 0){
              foreach($query->result() as $row)  
            {
           $action = '<button type="button" class="btn btn-sm btn-circle btn-primary btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
     ...
     function Approval_Rejected_Salesorder_DataTable($user_id){
        $query=$this->db->select('*,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y %r") as date_created,CONCAT(u.firstname, " ",u.lastname) AS sales_person')
            ->from('tbl_salesorder as s')->join('tbl_users as u','u.id=s.sales_person','LEFT')->where('s.status','REJECTED')->where('s.approval_status', 2)->order_by('date_created','DESC')->get();
           if($query !== FALSE && $query->num_rows() > 0){
              foreach($query->result() as $row)  
            {
           $action = '<button type="button" class="btn btn-sm btn-circle btn-primary btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->so_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
    ...
    function Approval_UsersRequest_DataTable($status){
     ...
     function Account_Report_Collection_Daily($month,$year){
            if($month == false || $year == false){
                $where = array('MONTH(date_deposite)'=> date('m'), 'YEAR(date_deposite)'=> date('Y'));
                $date = "MONTH(date_deposite)=".date('m')." AND YEAR(date_deposite)=".date('Y')."";
            }else{
                $where = array('MONTH(date_deposite)'=>$month,'YEAR(date_deposite)'=> $year);
                $date = "MONTH(date_deposite)=".$month." AND YEAR(date_deposite)=".$year."";
            }   
            $query = $this->db->select('*,CONCAT(firstname, " ",lastname) AS customer,DATE_FORMAT(date_deposite, "%M %d %Y") as date_created,
                (SELECT SUM(amount) FROM tbl_customer_deposite WHERE '.$date.') as total_amount')->from('tbl_customer_deposite')->where($where)->where('status','APPROVED')->order_by('date_deposite','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
             $i = 1;
             foreach($query->result() as $row)  
                {
                 ...
                 $total_gross = $row->total_amount / 1.12;
                 $total_vat = $row->total_amount - $total_gross;
                 $data[] = array(
                              'order_no'     => $row->order_no,
                              'si_no'        => $row->si_no,
       ...
       function Account_Report_Collection_Weekly($month,$year){
            ...
            $query = $this->db->select('*,CONCAT(firstname, " ",lastname) AS customer,
                ...
                ')->from('tbl_customer_deposite')->where($where)->where('status','APPROVED')
                 ->group_by('WEEK(date_deposite)')
                 ->order_by('WEEK(date_deposite)','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
                $i = 1;
             foreach($query->result() as $row)  
                {
     ...
     function Account_Report_Collection_Monthly($month,$year){
           if($month == false || $year == false){
                $where = array('YEAR(date_deposite)'=> date('Y'));
                $date = "YEAR(date_deposite)=".date('Y')."";
            }else{
                $where = array('YEAR(date_deposite)'=> $year);
                $date = "YEAR(date_deposite)=".$year."";
            }   
            $query = $this->db->select('*,sum(amount) as amount,CONCAT(firstname, " ",lastname) AS customer,DATE_FORMAT(date_deposite, "%M") as date_created,
                (SELECT SUM(amount) FROM tbl_customer_deposite WHERE '.$date.') as total_amount')->from('tbl_customer_deposite')->where($where)->where('status','APPROVED')->group_by('MONTH(date_deposite)')->order_by('date_deposite','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
             $i = 1;
             foreach($query->result() as $row)  
                {
      ...
      function Account_Report_Collection_Yearly($year){
           if($year == false){
            ...
            $query = $this->db->select('*,sum(amount) as amount,CONCAT(firstname, " ",lastname) AS customer,DATE_FORMAT(date_deposite, "%Y") as date_created, sum(amount) as amount,
                (SELECT SUM(amount) FROM tbl_customer_deposite WHERE '.$date.') as total_amount')->from('tbl_customer_deposite')->where($where)->where('status','APPROVED')->group_by('YEAR(date_deposite)')->order_by('YEAR(date_deposite)','DESC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
             $i = 1;
             foreach($query->result() as $row)  
                {
     ...
     function Account_Report_Salesorder_Daily($month,$year){
            if($month == false || $year == false){
                $where = array('MONTH(date_order)'=> date('m'), 'YEAR(date_order)'=> date('Y'));
                $date = "MONTH(date_order)=".date('m')." AND YEAR(date_order)=".date('Y')."";
            }else{
                $where = array('MONTH(date_order)'=>$month,'YEAR(date_order)'=> $year);
                $date = "MONTH(date_order)=".$month." AND YEAR(date_order)=".$year."";
            }   
            $query = $this->db->select('*,DATE_FORMAT(date_order, "%M %d %Y") as date_created,
               (SELECT SUM(total) FROM tbl_salesorder WHERE '.$date.') as total_subtotal,
               (SELECT SUM(shipping_fee) FROM tbl_salesorder WHERE '.$date.') as total_shippingfee,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12), 0)) AS total_vat FROM tbl_salesorder WHERE '.$date.') as total_vat,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee)) AS total_amount FROM tbl_salesorder WHERE '.$date.') as total_amount,
                IF(vat = "VATABLE",  (total*0.12), 0) AS vat,
                IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee) AS amount')->from('tbl_salesorder')->where($where)->where('status','DELIVERED')->order_by('date_order','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
             $i = 1;
             foreach($query->result() as $row)  
                {
                    $data[] = array('customer'          => $row->c_name,
                                    'si_no'             => $row->si_no,
                                    'subtotal'          => number_format($row->total,2),
                                    'vat'               => number_format($row->vat,2),
                                    'shipping_fee'      => number_format($row->shipping_fee,2),
                                    'amount'            => number_format($row->amount,2),
                                    'total_subtotal'    => number_format($row->total_subtotal,2),
                                    'total_vat'         => number_format($row->total_vat,2),
                                    'total_shippingfee' => number_format($row->total_shippingfee,2),
                                    'total_amount'      => number_format($row->total_amount,2),
                                    'date_created'      => $row->date_created);
                     $i++;
                }  
       ...
       function Account_Report_Salesorder_Weekly($month,$year){
            ...
            $query = $this->db->select('*,
                CONCAT("WEEK", " ",WEEK(date_order, 3) - WEEK(date_order - INTERVAL DAY(date_order)-1 DAY, 3) + 1)
                      as date_created,    
               (SELECT SUM(total) FROM tbl_salesorder WHERE '.$date.') as total_subtotal,
               (SELECT SUM(shipping_fee) FROM tbl_salesorder WHERE '.$date.') as total_shippingfee,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12), 0)) AS total_vat FROM tbl_salesorder WHERE '.$date.') as total_vat,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee)) AS total_amount FROM tbl_salesorder WHERE '.$date.') as total_amount,
                SUM(IF(vat = "VATABLE",  (total*0.12), 0)) AS vat,
                SUM(IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee)) AS amount,
                SUM(shipping_fee) as shipping_fee
                ')->from('tbl_salesorder')->where($where)->where('status','DELIVERED')
                 ->group_by('WEEK(date_order)')
                 ->order_by('WEEK(date_order)','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
                $i = 1;
             foreach($query->result() as $row)  
                {
     ...
     function Account_Report_Salesorder_Monthly($month,$year){
           if($month == false || $year == false){
                $where = array('YEAR(date_order)'=> date('Y'));
                $date = "YEAR(date_order)=".date('Y')."";
            }else{
                $where = array('YEAR(date_order)'=> $year);
                $date = "YEAR(date_order)=".$year."";
            }   
            $query = $this->db->select('*,
                DATE_FORMAT(date_order, "%M") as date_created,    
               (SELECT SUM(total) FROM tbl_salesorder WHERE '.$date.') as total_subtotal,
               (SELECT SUM(shipping_fee) FROM tbl_salesorder WHERE '.$date.') as total_shippingfee,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12), 0)) AS total_vat FROM tbl_salesorder WHERE '.$date.') as total_vat,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee)) AS total_amount FROM tbl_salesorder WHERE '.$date.') as total_amount,
                ...
                SUM(shipping_fee) as shipping_fee
                ')->from('tbl_salesorder')->where($where)->where('status','DELIVERED')
                 ->group_by('MONTH(date_order)')
                 ->order_by('MONTH(date_order)','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
                $i = 1;
             foreach($query->result() as $row)  
                {
      ...
      function Account_Report_Salesorder_Yearly($year){
           ...
           $query = $this->db->select('*,
                DATE_FORMAT(date_order, "%Y") as date_created,    
               (SELECT SUM(total) FROM tbl_salesorder WHERE '.$date.') as total_subtotal,
               (SELECT SUM(shipping_fee) FROM tbl_salesorder WHERE '.$date.') as total_shippingfee,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12), 0)) AS total_vat FROM tbl_salesorder WHERE '.$date.') as total_vat,
               (SELECT  SUM(IF(vat = "VATABLE",  (total*0.12+total+shipping_fee), total+shipping_fee)) AS total_amount FROM tbl_salesorder WHERE '.$date.') as total_amount,
                ...
                SUM(shipping_fee) as shipping_fee
                ')->from('tbl_salesorder')->where($where)->where('status','DELIVERED')
                 ->group_by('YEAR(date_order)')
                 ->order_by('YEAR(date_order)','ASC')->get();
             if($query !== FALSE && $query->num_rows() > 0){
                $i = 1;
             foreach($query->result() as $row)  
                {
    ...
    function Account_Report_Cashposition_Weekly($year,$month){
     ...
     //sales
     function OnlineOrder_DataTable($id,$status){
        if($status == 'REQUEST'){
            $array = array('s.status'=>$status);
        }else{
            $array = array('s.status'=>$status,'s.sales'=>$id);
        }
       $query = $this->db->select('s.*,u.*,s.type as type,s.status as status,DATE_FORMAT(s.date_order, "%M %d %Y") as date_created,CONCAT(u.firstname, " ",u.lastname) AS customer')->from('tbl_cart_address as s')->join('tbl_customer_online as u','u.id=s.customer','LEFT')->where($array)->order_by('date_order','DESC')->get();
      if($query !== FALSE && $query->num_rows() > 0){
         foreach($query->result() as $row)  
            {
            $action = '<button type="button" class="btn btn-sm btn-circle btn-primary btn-icon" data-toggle="modal" id="form-request" data-id="'.$row->order_no.'" data-status="approved" data-target="#requestModal"><i class="la la-eye"></i></button>';  
                $data[] = array(
                          'order_no'     => $row->order_no,
                          'customer'     => $row->customer,
                          'type'         => $row->type,
     ...
     function OnlineCustomization_DataTable($user_id,$status){
        $query =  $this->db->select('*,DATE_FORMAT(date_created, "%M %d %Y") as date_created')
            ->from('tbl_contact_us')->order_by('date_created','DESC')->get();
        $s = 1;
        if($query !== FALSE && $query->num_rows() > 0){
           foreach($query->result() as $row){
     ...
     function Customer_Concern_DataTable($id,$status){
       ...
       function Customer_Deposite_DataTable(){
          $query = $this->db->select('*,CONCAT(firstname, " ",lastname) AS customer,DATE_FORMAT(date_deposite, "%M %d %Y") as date_created')->from('tbl_customer_deposite')->where('status IS NULL')->order_by('date_created','DESC')->get();
         if($query !== FALSE && $query->num_rows() > 0){
            $i = 1;
         foreach($query->result() as $row)  
            {
      ...
      function Customer_Collected_DataTable(){
