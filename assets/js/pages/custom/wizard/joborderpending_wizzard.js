		...
		_wizardObj.on('submit', function (wizard) {
			...
			if (!rowCount) {
				_initSwalWarning();
				    ...
				    let production_no = $('input[name=production_no]').val();
				    let production = $('input[name=production]').val();
				    val = {production_no:production_no,production:production,mat_type,mat_itemno,mat_item:mat_name,mat_quantity:mat_quantity,mat_unit:mat_unit,mat_remarks:mat_remarks,pur_item:pur_item,pur_quantity:p_quantity,pur_unit:p_unit,pur_remarks:p_remarks,pur_type:p_type};
				    thisURL = baseURL + 'update_controller/Update_Joborder_Pending';
				    let role = $('input[name=role]').val();
				    if(role == 'production'){
				    	 url = baseURL + 'gh/production/joborder';
				    }else if(role == 'superuser'){
				    	 url = baseURL + 'gh/superuser/joborder';
				    }else if(role == 'admin'){
				    	 url = baseURL + 'gh/admin/joborder';
				    }else if(role == 'designer'){
				    	 url = baseURL + 'gh/designer/joborder';
				    }
