...
var KTApexChartsDemo = function () {
	...
	var _demo14 = function () {
		...
		var _gensale = function (graph,type,option=null,year=null,month=null) {
        ...
        $.ajax({
            url: baseURL+"Chart_controller/overview_gensales",
            type: "POST",
            data:{type:type,option:option,year:year,month:month},
            dataType: "json",
            beforeSend: function(){
            // KTApp.blockPage();
            },
			...
			legend: {
             ...
             success:function(response){
                if(response != false){
                  if(response.gensales.length >= 1){
                     for(var i = 0; response.gensales.length > i; i++){
						gensales.push(Number(response.gensales[i].amount));
                        if(option == "MONTH"){
                      ...
                      for(var i = 0; response.expenses.length > i; i++){
						expenses.push(Number(response.expenses[i].amount));
                        if(option == "MONTH"){
                           months.push(response[i].label_month.substring(0, 3));
                        }else if(option == "WEEK"){
                           months.push('Week '+response[i].options+' of '+response[i].label_month.substring(0, 3));
	...
	return {
		// public functions
		init: function () {
			var chart = $('#kt_content').attr('data-table');
			if(chart=="data-dashboard-accounting"){
                _gensale('chart1','all_options');
            }
			_demo1();
			_demo2();
			_demo3();
